#!/usr/bin/env Rscript

# Flexible unit tests for R package versions

# Source the configuration
config_paths <- c(
  "/workspace/package_test_config.R",
  "package_test_config.R",
  file.path(getwd(), "package_test_config.R")
)

for (path in config_paths) {
  if (file.exists(path)) {
    source(path)
    break
  }
}

parse_r_packages <- function() {
  # Parse r-packages.txt and extract package requirements
  # Try multiple locations for r-packages.txt
  possible_paths <- c(
    "/tmp/r-packages.txt",  # During Docker build
    "r-packages.txt",       # Current directory
    file.path(getwd(), "r-packages.txt")  # Working directory
  )
  
  packages_file <- NULL
  for (path in possible_paths) {
    if (file.exists(path)) {
      packages_file <- path
      break
    }
  }
  
  if (is.null(packages_file)) {
    stop("Could not find r-packages.txt in any expected location")
  }
  
  lines <- readLines(packages_file)
  
  # Filter out comments and empty lines
  package_lines <- lines[!grepl("^#", lines) & nzchar(trimws(lines))]
  
  # Parse package names and versions
  packages <- list()
  for (line in package_lines) {
    parts <- strsplit(trimws(line), "@")[[1]]
    package_name <- parts[1]
    package_version <- if(length(parts) > 1) parts[2] else NA
    packages[[package_name]] <- package_version
  }
  
  return(packages)
}

get_installed_version <- function(package_name) {
  # Get the installed version of a package
  if (package_name %in% rownames(installed.packages())) {
    return(as.character(packageVersion(package_name)))
  } else {
    return(NULL)
  }
}

# Function to normalize version strings
normalize_version <- function(version) {
  # Handle common version format differences
  version <- gsub("-", ".", version)  # Convert 3.5-7 to 3.5.7
  return(version)
}

test_r_packages <- function() {
  cat(paste(rep("=", 60), collapse=""), "\n")
  cat("Testing R Package Versions (Flexible Mode)\n")
  cat(paste(rep("=", 60), collapse=""), "\n")
  
  expected_packages <- parse_r_packages()
  
  # Different categories of results
  critical_failures <- list()
  warnings <- list()
  passed_tests <- character()
  optional_missing <- character()
  
  for (package_name in names(expected_packages)) {
    expected_version <- expected_packages[[package_name]]
    installed_version <- get_installed_version(package_name)
    severity <- get_test_severity(package_name)
    
    if (is.null(installed_version)) {
      # Package not installed
      if (severity == "optional") {
        optional_missing <- c(optional_missing, package_name)
        cat(sprintf("ℹ️  %s: NOT INSTALLED (optional package)\n", package_name))
      } else {
        critical_failures[[length(critical_failures) + 1]] <- list(
          package = package_name,
          expected = expected_version,
          actual = "NOT INSTALLED",
          status = "MISSING",
          severity = severity
        )
        cat(sprintf("❌ %s: NOT INSTALLED (expected %s)\n", package_name, 
                    ifelse(is.na(expected_version), "any version", expected_version)))
      }
    } else if (!is.na(expected_version)) {
      # Check version compatibility
      if (is_version_acceptable(package_name, installed_version, expected_version)) {
        passed_tests <- c(passed_tests, package_name)
        if (normalize_version(installed_version) != normalize_version(expected_version)) {
          cat(sprintf("✅ %s: %s (expected %s, newer version OK)\n", 
                      package_name, installed_version, expected_version))
        } else {
          cat(sprintf("✅ %s: %s\n", package_name, installed_version))
        }
      } else {
        # Version mismatch
        if (severity %in% c("bioconductor", "flexible")) {
          warnings[[length(warnings) + 1]] <- list(
            package = package_name,
            expected = expected_version,
            actual = installed_version,
            severity = severity
          )
          cat(sprintf("⚠️  %s: %s (expected %s)\n", 
                      package_name, installed_version, expected_version))
          passed_tests <- c(passed_tests, package_name)
        } else {
          critical_failures[[length(critical_failures) + 1]] <- list(
            package = package_name,
            expected = expected_version,
            actual = installed_version,
            status = "VERSION MISMATCH",
            severity = severity
          )
          cat(sprintf("❌ %s: %s (expected %s)\n", 
                      package_name, installed_version, expected_version))
        }
      }
    } else {
      # No version requirement
      passed_tests <- c(passed_tests, package_name)
      cat(sprintf("✅ %s: %s\n", package_name, installed_version))
    }
  }
  
  cat("\n", paste(rep("=", 60), collapse=""), "\n")
  cat(sprintf("Summary: %d passed, %d warnings, %d critical failures\n", 
              length(passed_tests), length(warnings), length(critical_failures)))
  if (length(optional_missing) > 0) {
    cat(sprintf("         %d optional packages not installed\n", length(optional_missing)))
  }
  cat(paste(rep("=", 60), collapse=""), "\n")
  
  if (length(critical_failures) > 0) {
    cat("\nCritical failures:\n")
    for (failure in critical_failures) {
      cat(sprintf("  - %s: %s\n", failure$package, failure$status))
      cat(sprintf("    Expected: %s\n", failure$expected))
      cat(sprintf("    Actual: %s\n", failure$actual))
    }
  }
  
  if (length(warnings) > 0) {
    cat("\nWarnings (non-critical):\n")
    for (warning in warnings) {
      cat(sprintf("  - %s: %s (expected %s)\n", 
                  warning$package, warning$actual, warning$expected))
    }
  }
  
  # Only fail if there are critical failures
  return(length(critical_failures) == 0)
}

# Run the tests
if (!interactive()) {
  success <- test_r_packages()
  quit(save = "no", status = ifelse(success, 0, 1))
}